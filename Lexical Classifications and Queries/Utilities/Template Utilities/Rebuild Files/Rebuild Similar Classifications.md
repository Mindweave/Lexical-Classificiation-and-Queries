---
Aliases: 
NoteType: SimilarQuerySearch

hasFeatures: hasDataviewQuery 
---
(Template:: [[Utilities/Template Utilities/Rebuild Files/Rebuild Similar Classifications.md|Rebuild Similar Classifications]] )

# Search Similar Classifications
...
(Only_including_these_file_paths:: "Scraps")
...
return ObjectValues.includes(stringValFue)
}
...
function findFiles(anyClassificationLinks,allClassificationLinks,excludingAnyClassificationLinks,excludingAllClassificationLinks,onlyFilesToSearch,doNotSearchTheseFiles,createdDateBefore,createdDateAfter,includingFilePaths,excludingFilePaths,fileSearchingOn){
...
//check if it is the file being searched
if(p.file.path == fileSearchingOn){
return false;
}
...
let hasAtLeastOneFilter = (anyClassificationLinksIsFiltered || allClassificationLinksIsFiltered || excludingAnyClassificationLinksIsFiltered || excludingAllClassificationLinksIsFiltered || createdDateBeforeIsFiltered || createdDateAfterIsFiltered || excludingFilePathsIsFiltered || includingFilePathsIsFiltered || doNotSearchFilesIsFiltered || onlyFilesIsFiltered)
...
var mostRecentFilePath = "VarStart_NoteSourcePath_VarEnd"

let sourceFile = dv.page(mostRecentFilePath);
let sourceFileClassifications = typeof(sourceFile?.found_classifications) == "undefined"? []:pathsFromListOfLinks(sourceFile.found_classifications);
let sourceFileClassificationsLength = typeof(sourceFileClassifications?.length) == "undefined" || sourceFileClassifications?.length == 0  ? 1:  sourceFileClassifications?.length; //so it does not divide by zero

dv.el('p',"SourceFile: "+sourceFile.file.link);
...
let groupOne = findFiles(anyClassificationLinks,allClassificationLinks,excludingAnyOfTheseClassificationLinks,excludingAllOfTheseClassificationLinks,onlyFilesToSearch,doNotSearchTheseFiles,createdBefore,createdAfter,includingFilePaths,excludingFilePaths,mostRecentFilePath);

dv.table(["Compared Link","% Matching Source File","% Only in Compared Link"],(
    groupOne).map(a => {
    let foundClassificationsPaths = pathsFromListOfLinks(a.found_classifications)
    let matchingSourceFile = foundClassificationsPaths.filter( found => {return objectPartiallyMatchesObject(sourceFileClassifications,[found])} )
    let outsideSourceFile = foundClassificationsPaths.length - matchingSourceFile.length;
    let percentageOutside = ((outsideSourceFile / foundClassificationsPaths.length) * 100).toFixed(0)+"%"; 
	let percentageMatching = ((matchingSourceFile.length / sourceFileClassificationsLength) * 100).toFixed(0)+"%";
return [(a.file.link),percentageMatching,percentageOutside]}))
